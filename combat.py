#Taken from https://github.com/brentp/combat.py/blob/master/combat.py
...
def combat(data, batch, model=None, numerical_covariates=None):
    ...
    # drop intercept
    drop_cols = [cname for cname, inter in ((model == 1).all()).iteritems()
                 if inter is True]
    ...
    numerical_covariates = [list(model.columns).index(c) if isinstance(c, str)
                            else c for c in numerical_covariates
